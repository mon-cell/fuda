...
def insert_text(seq_obj):
    ...
    if isinstance(seq_obj, Hai_List):
        st.session_state['seq_list'].append(chr(int(seq_obj.name, 16)))
        st.session_state['text'] += chr(int(seq_obj.name, 16))

def order_text():
    if 'seq_list' in st.session_state:
        print('Before: ', [seq.name for seq in st.session_state['seq_list']])
        sorted_list = sorted(st.session_state['seq_list'], key=lambda x: x.no)
        sorted_text = ''.join([seq.name for seq in sorted_list])
        st.session_state['text'] = sorted_text
        
#         st.session_state['seq_list'] = [
#             obj for obj in st.session_state['seq_list'] if isinstance(obj, Hai_List)
#         ]
        
        
    #print(f'seq_list')
        print(f'Sorted: ', sorted_text)
        #st.write(f'Sorted: {sorted_text}')
...
if 'text_no' not in st.session_state:
    st.session_state['text_no'] = ''

st.text_area("no seq", key='text_no', height=50)
                                    
                                    
col_m1, col_m2, col_m3 = st.columns(3)
with col_m1:
    st.button(chr(int(m1.name, 16)), on_click=insert_text, args=(m1,))    
with col_m2:
    st.button(chr(int(m2.name, 16)), on_click=insert_text, args=(m2,))
with col_m3:
    st.button(chr(int(m3.name, 16)), on_click=insert_text, args=(m3,))
...
st.button('Order', on_click=order_text)
